...
class PortfolioManagementEIIEEnvironment(Environments):
    ...
    def reset(self):
        ...
        self.asset_memory = [self.initial_amount]
        self.portfolio_return_memory = [0]
        self.weights_memory = [[1 / (self.stock_dim + 1)] * (self.stock_dim + 1)]
        self.date_memory = [self.data.date.unique()[0]]
        self.transaction_cost_memory = []
    ...
    def step(self, weights):
        ...
        if self.terminal:
            ...
            metric_save_path = osp.join(
                self.work_dir,
                "metric_"
                + str(self.task)
                + "_"
                + str(self.test_dynamic)
                + "_"
                + str(self.test_id)
                + "_"
                + str(self.task_index)
            ...
            portfolio_return = sum(
                ((new_price_memory.close.values / last_day_memory.close.values) - 1)
                * portfolio_weights
            )
            ...
            # Ensure portfolio value never goes below a small epsilon to prevent numerical issues
            new_portfolio_value = max(new_portfolio_value, 1e-10)
            portfolio_return = (
                new_portfolio_value - self.portfolio_value
            ) / self.portfolio_value
            self.reward = np.log(max(new_portfolio_value, 1e-10)) - np.log(
                max(self.portfolio_value, 1e-10)
            )
        ...
        return (
            ...
            self.reward,
            self.terminal,
            {"weights_brandnew": weights_brandnew},
        )
    ...
    def evaualte(self, df):
        ...
        sharpe_ratio = (
            np.mean(return_rate_list)
            * (252) ** 0.5
            / (np.std(return_rate_list) + 1e-10)
        )
